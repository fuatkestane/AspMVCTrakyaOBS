    ...
    public class UyelikRepository
    {
        ...
        public NIslemSonuc<string> KullaniciEkle (string kullaniciAd, string rol)
        {
            ...
            try
            {
                ...
                UserManager.AddPassword(kullanici.Id, rol);
                ...
                {
                    return new NIslemSonuc<string> { BasariliMi = false, Mesaj = "Kullanıcıya rol eklemesi yapılamadı" };
                }
            }
        ...
        public NIslemSonuc<NSession> GirisYap(string kullaniciAdi, string sifre, Enumlar.UyeTip tip)
        {
            NSession session = new NSession();
            if (tip == Enumlar.UyeTip.Ogrenci)
            {
            ...
            else if (tip == Enumlar.UyeTip.OgretimGorevlisi)
            {
            ...
            else if (tip == Enumlar.UyeTip.BilgiIslem)//Bilgi işlem kullanıcısı öğrenci veya öğretim görevlisi olamaz
            {
                OgrenciRepository ogrenciRepository = new OgrenciRepository(false);
                var ogrenci = ogrenciRepository.Getir(kullaniciAdi);
                if (ogrenci.BasariliMi)
